...
public sealed class DapperDatabaseInitializer(IDbConnectionFactory connectionFactory) : IDatabaseInitializer
{
    ...
    private const string Sql = """
        ...
        create table if not exists watchlist_data (
            ...
            day_low numeric(18, 4) null,
            day_high numeric(18, 4) null,
            average_price numeric(18, 4) null,
            price_sample_count integer not null default 0,
            final_price numeric(18, 4) null,
            first_price_at_utc timestamptz null,
            last_price_at_utc timestamptz null,
            created_at_utc timestamptz not null,
            updated_at_utc timestamptz null
        );

        create unique index if not exists ux_watchlist_data_watchlist_date
            on watchlist_data (watchlist_id, trading_date);

        create unique index if not exists ux_watchlist_data_stock_date
            on watchlist_data (stock_id, trading_date);

        create index if not exists ix_watchlist_data_stock_id
            on watchlist_data (stock_id);

        alter table watchlist_data
            add column if not exists day_low numeric(18, 4) null;

        alter table watchlist_data
            add column if not exists day_high numeric(18, 4) null;

        alter table watchlist_data
            add column if not exists average_price numeric(18, 4) null;

        alter table watchlist_data
            add column if not exists price_sample_count integer not null default 0;

        alter table watchlist_data
            add column if not exists first_price_at_utc timestamptz null;

        alter table watchlist_data
            add column if not exists last_price_at_utc timestamptz null;
